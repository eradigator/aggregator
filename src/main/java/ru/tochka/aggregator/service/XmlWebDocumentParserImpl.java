...
@Service
@Slf4j
public class XmlWebDocumentParserImpl implements WebDocumentParser {
    ...
    public List<Post> parse(Website website) {
        ...
        try {
            ...
            items.forEach(item -> {
                String title = item.getElementsByTag(postTitleClass).text();
                String body = item.getElementsByTag(postBodyClass).text();
